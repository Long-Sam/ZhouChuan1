...
public class SysButtons : MonoBehaviour {
    ...
    void Start()
    {
        ...
        buttons[1].onClick.AddListener(delegate { for (int i = 0; i < mianban.Length; i++) { mianban[i].gameObject.SetActive(false); } mianban[1].gameObject.SetActive(true); for (int i = 0; i < mianban[1].transform.childCount; i++) { mianban[1].transform.GetChild(i).gameObject.SetActive(false); } mianban[1].transform.GetChild(0).gameObject.SetActive(true); });
        buttons[2].onClick.AddListener(delegate { for (int i = 0; i < mianban.Length; i++) { mianban[i].gameObject.SetActive(false); } mianban[2].gameObject.SetActive(true); for (int i = 0; i < mianban[2].transform.childCount; i++) { mianban[2].transform.GetChild(i).gameObject.SetActive(false); } mianban[2].transform.GetChild(0).gameObject.SetActive(true); });
